...
采集器需要 Node.js 24+、npm 与 Herdr 0.9.1+；实时输入当前要求 Herdr 0.9.1 的协议 22。可从 npm 独立安装，无须克隆 Eagle 仓库。网站与 Agent 统一使用 v0.5.0：
...
预期版本为 `0.5.0`。命令仅为本次安装指定源，不修改全局 npm 源；镜像可能延迟同步，遇到 `404` / `ETARGET` 时稍后重试，保持固定版本。按照[安装与配置说明](agent/README.md)保存接入凭据后运行：
