...
| `GET /api/v1/overview` | Viewer Bearer or cookie | Server time, latest report per machine, last heartbeat and collector warning |
| `GET /api/v1/history` | Viewer Bearer or cookie | Optional `machine`, `space`, `before` cursor, `limit` 1–100 (default 20); entries and nextCursor |
