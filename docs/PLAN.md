...
- Executive overview, topology, pane evidence drilldown, DO current changes and existing paginated D1 history. Raw snapshot D1 writes remain paused. AI hourly reports were subsequently authorized and are implemented as a separate next-ai aggregation/archive flow; see HOURLY-REPORTS.md. Browser refresh every five seconds with failure and freshness visibility.
