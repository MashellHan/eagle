...
The collector requires Node.js 24+, npm and Herdr 0.9.1+; realtime input currently requires Herdr 0.9.1 protocol 22. Install it independently from npm without cloning Eagle. The website and Agent share release v0.6.0:
...
The expected version is `0.6.0`. These commands select a registry for this installation only, without changing the global npm registry. Mirrors may lag (`404` / `ETARGET`); retry later and keep the pinned version. Save onboarding credentials following the [installation and configuration guide](../agent/README.md), then run:
