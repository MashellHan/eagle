...
Install Agent v0.5.1 from npm. Website and Agent releases use the same version from the root package manifest. Check `node --version`, `npm --version` and `herdr --version`: Node 24+ and a running Herdr installation are required. Node downloads: https://nodejs.org/en/download. The installed Agent needs no Eagle checkout or TypeScript compiler.
